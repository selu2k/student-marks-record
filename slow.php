...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="restable">
                <?php
                $sqlavg = "SELECT AVG(internal) FROM student";
                $resultavg = mysqli_query($conn, $sqlavg);
                $rowavg = mysqli_fetch_assoc($resultavg);
                $avg = $rowavg['AVG(internal)'];
                $avg = round($avg, 2);
                echo "<h2 style='text-align:center; margin-top:20px;'>Students with marks less than average($avg)</h2>";
                $update = "UPDATE student SET comment='Slow Learner' WHERE internal<$avg";
                $resultupdate = mysqli_query($conn, $update);
                $sql = "SELECT * FROM student WHERE internal < $avg";
